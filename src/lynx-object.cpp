...
Self Object::__pow__(Args args){
    ...
    if(lhs->is_number() && rhs->is_number()){
        if(lhs->is_integer() && rhs->is_integer()){
            auto x = std::get<i64>(lhs->m_value);
            auto y = std::get<i64>(rhs->m_value);
            obj = Object(static_cast<i64>(std::pow(f64(x), f64(y))));
        }else if(lhs->is_float() && rhs->is_float()){
            auto x = std::get<f64>(lhs->m_value);
            auto y = std::get<f64>(rhs->m_value);
            obj = Object(std::pow(x, y));
        }else if(lhs->is_complex() && rhs->is_complex()){
            auto x = std::get<Complex>(lhs->m_value);
            auto y = std::get<Complex>(rhs->m_value);
            obj = Object(std::pow(x, y));
...
Self Object::__eq__(Args args){
    ...
    if(lhs->is_bool() && rhs->is_bool()){
        ...
        auto xstr = std::get<Symbol>(lhs->m_value).str();
        auto ystr = std::get<Symbol>(rhs->m_value).str();
        obj = Object((xstr == ystr));
    }else{
        auto val = *Object()("==", args);
        obj = Object(val);
...
Self Object::__float__(Args args){
    if(check_argcount(args, 1)){
        std::stringstream stream;
        stream << "SyntaxError: `__float__` : invalid number of arguments\n";
        stream << "Expected 1 arguments, but got " << args.size();
    }
    auto self = args[0];
    Object obj{};
    if(self->is_bool() || self->is_integer() || self->is_float()){
        auto val = static_cast<f64>(*self);
        obj = Object(val);
    }else{
        Args argv{};
        argv.push_back(std::make_shared<Object>(*self));
        argv.push_back(std::make_shared<Object>("__float__"));
        auto flag = static_cast<bool>(*Object().hasattr(argv));
        if(!flag){
            std::stringstream stream;
            stream << "ValueError: argument is not convertible to integer";
            ...
            auto val = static_cast<f64>(*Object()("__float__", args));
            obj = Object(val);
        }
    }
...
Self Object::__list__(Args args){
    if(check_argcount(args, 1)){
        std::stringstream stream;
        stream << "SyntaxError: `__list__` : invalid number of arguments\n";
        stream << "Expected 1 arguments, but got " << args.size();
    }
    auto self = args[0];
    Object obj{};
    if(self->is_string()){
        ...
        Args argv{};
        argv.push_back(std::make_shared<Object>(*self));
        argv.push_back(std::make_shared<Object>("__list__"));
        auto flag = static_cast<bool>(*Object().hasattr(argv));
        if(!flag){
            std::stringstream stream;
            stream << "ValueError: argument is not convertible to integer";
            ...
            auto val = static_cast<List>(*Object()("__list__", args));
            obj = Object(Object::Kind::Vector, val);
        }
    }
